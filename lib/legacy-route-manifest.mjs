...
const legacyHtmlAliasFiles = migratedPublicHtmlRewrites
  .map(({ source }) => String(source || "").replace(/^\//, ""))
  .filter((file) => file.endsWith(".html"));

export const legacyCopiedHtmlFiles = [...legacyGuideHtmlFiles];

export const legacyManagedFiles = Array.from(
  new Set([
    ...legacyHtmlAliasFiles,
...
export const legacyCopiedFiles = Array.from(
  new Set([
    ...legacyCopiedHtmlFiles,
    ...legacyJavaScriptAssets,
    ...legacyStyleAssets,
  ])
).sort((left, right) => left.localeCompare(right));
...
export const noIndexLegacyPaths = Array.from(
  new Set([
    ...
    "/bases/:path*",
    "/app/:path*",
    ...legacyGuideHtmlFiles.map((file) => `/${file}`),
  ])
).sort((left, right) => left.localeCompare(right));
