...
const isProduction = () => normalizeEnv("NODE_ENV") === "production";
...
const getFeedbackIpHashSalt = () => {
  const hashSalt =
    normalizeEnv("FEEDBACK_IP_HASH_SALT") || normalizeEnv("LEGAL_IP_HASH_SALT");

  if (!hashSalt && isProduction()) {
    throw new Error(
      "FEEDBACK_IP_HASH_SALT (or LEGAL_IP_HASH_SALT) is required in production to hash client IP addresses safely."
    );
  }
...
const buildRequestIpHash = (request) => {
  ...
  const hashSalt = getFeedbackIpHashSalt();
  if (!hashSalt) {
    return null;
  }
...
const loadRecentFeedbackCount = async ({ admin, column, value, windowStartIso }) => {
...
const enforceFeedbackRateLimit = async ({ admin, ipHash, userId }) => {
  ...
  const windowStartIso = new Date(Date.now() - ONE_HOUR_MS).toISOString();
  const checks = [];

  if (ipHash) {
    checks.push(loadRecentFeedbackCount({ admin, column: "ip_hash", value: ipHash, windowStartIso }));
  }

  if (userId) {
    checks.push(loadRecentFeedbackCount({ admin, column: "user_id", value: userId, windowStartIso }));
  }

  const counts = await Promise.all(checks);
  const highestObservedCount = Math.max(0, ...counts);

  if (highestObservedCount >= MAX_SUBMISSIONS_PER_HOUR) {
